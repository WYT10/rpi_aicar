...
if TORCH_AVAILABLE:
    class TinySteerNet(nn.Module):
        ...
        Works with arbitrary HxW; fc in_features are computed at init.
        """
...
class CameraManager:
    ...
    def _apply_processing(self, frame_bgr):
        # NOTE: Removed the flip here. Backends return BGR, and we want BGR.
...
control_state = {
    # direct human command (joystick / UI)
    "human_left": 0.0,
    "human_right": 0.0,

    # raw model policy output from TinySteerNet
    "model_left": 0.0,
    "model_right": 0.0,

    # final logical motor command after blending & gains
    "left_target": 0.0,
    "right_target": 0.0,

    # smoothed command that motor loop actually uses
    "left_actual": 0.0,
    "right_actual": 0.0,

    # last time any target was updated
...
def motor_loop(motors: MotorController):
    ...
    while motor_loop_running:
        ...
        now = time.time()
        idle_timeout = 0.5  # seconds of no updates => stop motors
        if now - control_state.get("last_update_ts", now) > idle_timeout:
            lt = 0.0
            rt = 0.0
...
camera = CameraManager(
    width=cam_cfg.get("width", 224),
    height=cam_cfg.get("height", 224),
...
deploy_status = {
    ...
    # if False, autopilot computes but motors stay idle
    "drive_motors": False,

    # V2: shared-control mode
    # "auto"   = pure model
    # "assist" = blend human + model
    "mode": autopilot_cfg.get("default_mode", "assist"),
    "assist_blend": float(autopilot_cfg.get("default_assist_blend", 0.7)),

    # simple safety / reflex layer
    "reflex_on": bool(autopilot_cfg.get("reflex_on", True)),
    "min_brightness": float(autopilot_cfg.get("min_brightness", 18.0)),

    # diagnostics
    "note": "",
    "confidence": None,   # future: policy confidence
}
...
autopilot_image_size = (
    int(train_cfg.get("image_width", 224)),
    int(train_cfg.get("image_height", 224)),
)
...
def api_motors_openloop():
    """
    Human open-loop command.

    - Always writes to human_left/human_right.
    - If autopilot is OFF, human drives the targets directly.
    - If autopilot is ON and mode='assist', human is blended with model
    ...
    with control_lock:
        control_state["human_left"] = left
        control_state["human_right"] = right

        # If autopilot isn't actively driving, human takes full control
...
def api_motors_stop():
    with control_lock:
        control_state["human_left"] = 0.0
        control_state["human_right"] = 0.0
        control_state["model_left"] = 0.0
        control_state["model_right"] = 0.0
...
def api_train_start():
    ...
    w = int(body.get("image_width", train_cfg.get("image_width", 224)))
    h = int(body.get("image_height", train_cfg.get("image_height", 224)))
    image_size = (w, h)

    class FilenameDataset(Dataset):
        def __init__(self, ds_name: str, image_size=(224, 224)):
    ...
    def _run_train():
        ...
        try:
            ...
            # Instantiate model with dynamic size
            model = TinySteerNet(image_height=image_size[1], image_width=image_size[0]).to(device)
            criterion = nn.MSELoss()
            optimizer = optim.Adam(model.parameters(), lr=lr)

            logger.info(
                "Train: dataset=%s, samples=%d, epochs=%d, batch=%d, lr=%.4f, device=%s, size=%s",
                dataset,
                len(ds),
                epochs,
                batch_size,
...
def api_models_convert():
    ...
    try:
        ...
        model = TinySteerNet(image_height=image_size[1], image_width=image_size[0])
...
def load_policy_model(name: str):
    ...
    try:
        ...
        logger.exception("load_policy_model: failed to load %s", path)
        return False, str(e)
    ...
    image_size = tuple(ckpt.get("image_size", autopilot_image_size))
    dtype = ckpt.get("dtype", "float32")

    if TinySteerNet is None:
        return False, "TinySteerNet not available"

    # Build correct model type
    if dtype == "int8_dynamic" and quantize_dynamic is not None:
        base = TinySteerNet(image_height=image_size[1], image_width=image_size[0])
        model = quantize_dynamic(base, {nn.Linear}, dtype=torch.qint8)
        logger.info("load_policy_model: using int8_dynamic quantized TinySteerNet")
    else:
        model = TinySteerNet(image_height=image_size[1], image_width=image_size[0])
        logger.info("load_policy_model: using float TinySteerNet (dtype=%s)", dtype)

    try:
        model.load_state_dict(model_state)
    except Exception as e:
        logger.exception("load_policy_model: state_dict load failed")
        return False, f"state_dict mismatch: {e}"
    ...
    autopilot_image_size = (int(image_size[0]), int(image_size[1]))
    logger.info(
        "Policy model loaded: %s (device=%s, image_size=%s, dtype=%s)",
        name,
        device,
        autopilot_image_size,
        dtype,
...
def run_policy_on_frame(frame_bgr):
    ...
    small = cv2.resize(frame_bgr, (w, h), interpolation=cv2.INTER_LINEAR)
...
def _lr_to_click(left: float, right: float):
    """
    Approximate inverse of the label mapping.

    - left,right in [0,1]
    - returns integer (X,Y) on a 150x150 grid
      X: 0..149 left->right
      Y: 0..149 top->bottom (0 = forward, 149 = stop)
    """
    ...
    s = left + right
    if s <= 1e-3:
        # basically stopped: bottom-center
        return 75, 149

    # forward approx = mean(left,right)
    forward = max(0.0, min(1.0, 0.5 * s))

    # steer approx = (right-left)/(left+right)
    steer = (right - left) / max(s, 1e-6)
    steer = max(-1.0, min(1.0, steer))

    # map steer ∈ [-1,1] -> sx ∈ [0,1]
    sx = 0.5 + 0.5 * steer
    ...
    X = int(round(sx * 149.0))
    Y = int(round(sy * 149.0))
    return X, Y
...
def _autopilot_loop():
    ...
    while deploy_status["autopilot"]:
        ...
        base_left = max(0.0, min(1.0, base_left))
        base_right = max(0.0, min(1.0, base_right))

        # --- 2. frame quality for reflex safety ---
        try:
        ...
        mode = deploy_status.get("mode", "assist")
        assist_blend = float(deploy_status.get("assist_blend", 0.7))
        assist_blend = max(0.0, min(1.0, assist_blend))

        reflex_on = bool(deploy_status.get("reflex_on", True))
        min_brightness = float(deploy_status.get("min_brightness", 18.0))

        with control_lock:
            human_left = float(control_state.get("human_left", 0.0))
            human_right = float(control_state.get("human_right", 0.0))

        # --- 4. blend human vs model ---
        if mode == "assist":
            left = (1.0 - assist_blend) * human_left + assist_blend * base_left
            right = (1.0 - assist_blend) * human_right + assist_blend * base_right
        else:  # "auto"
            left, right = base_left, base_right

        # --- 5. reflex: stop if too dark ---
        if reflex_on and (brightness is not None) and (brightness < min_brightness):
            left = 0.0
            right = 0.0
            deploy_status["note"] = f"reflex: dark frame ({brightness:.1f})"
        else:
            deploy_status["note"] = ""

        # --- 6. apply gains & expo ---
        left = max(0.0, min(1.0, left * throttle_gain))
        right = max(0.0, min(1.0, right * throttle_gain))
        ...
        # --- 7. write back to control_state ---
        with control_lock:
            # keep raw model for debugging / future learning
            control_state["model_left"] = base_left
            control_state["model_right"] = base_right
...
def api_deploy_gains():
    ...
    mode = body.get("mode", deploy_status.get("mode", "assist"))
    if mode not in ("auto", "assist"):
        mode = deploy_status.get("mode", "assist")

    assist_blend = float(body.get("assist_blend", deploy_status.get("assist_blend", 0.7)))
    assist_blend = max(0.0, min(1.0, assist_blend))

    reflex_on = bool(body.get("reflex_on", deploy_status.get("reflex_on", True)))
    min_brightness = float(body.get("min_brightness", deploy_status.get("min_brightness", 18.0)))
    ...
    deploy_status["mode"] = mode
    deploy_status["assist_blend"] = assist_blend
    deploy_status["reflex_on"] = reflex_on
    deploy_status["min_brightness"] = min_brightness
...
def api_deploy_test_frame():
    ...
    # V2: map (left,right) to a real intent point on the 150x150 grid
    X, Y = _lr_to_click(left, right)
    ...
    return ok(
        {
            ...
            "left": float(left),
            "right": float(right),
            "note": "TinySteerNet inference; X/Y derived from (left,right).",
        }
    )
